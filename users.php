...
{
    $sql = "INSERT INTO userdetail (email, username)
...
{
    $trp = mysqli_query($conn, "SELECT * from userdetail where id =".$_POST['userid']);
    $rows = array();
...
else
{
    ...
    $trp = mysqli_query($conn, "SELECT * from userdetail ORDER BY id DESC");
    $rows = array();
    while($r = mysqli_fetch_assoc($trp)) {
        $rows[] = $r;
    }
